from samet_toolkit.llm_ops.embeddings import BaseEmbeddingGenerator, MAX_BATCH_SIZE
import cohere


class EmbeddingGeneratorCohere(BaseEmbeddingGenerator):
    def __init__(
        self,
        texts: list[str] | str,
        model_name: str,
    ...
    def generate_embeddings(self, max_batch=MAX_BATCH_SIZE) -> list[list[float]]:
        ...
        for i in range(0, num_texts, max_batch):
            ...
            all_embeddings_obj = self.client.embed(
                texts=text_batch, model=self.model_name, input_type=self.input_type
            )
    ...
    def _get_embedding_client(self, embedding_client):
        if not embedding_client:
            embedding_client = cohere.Client(api_key=self.config.secrets.cohere_api_key)
        return embedding_client

    def _detect_input_type(self):
        return (
            self.params.get("input_type")
            if self.params.get("input_type") in ["search_document", "search_query"]
